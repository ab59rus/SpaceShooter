...
#спрайты
spaceShip = Hero('rocket.png', int(win_width/2), int(win_height - win_width/9), 5)
...
for i in range(6):
    ememy = Enemy('ufo.png', randint(int(win_width/10), int(win_width*0.9)), randint(-100, 0), randint(1,2))
    enemies.add(ememy)
btn_start = Button(
...
while run:
    ...
    if mode == 'menu':
        ...
        res_time -= 1
        main_win.blit(res_text, (175,220))
        if res_time <= 0:
            mode = 'menu'
